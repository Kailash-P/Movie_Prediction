...
movieMap = {0:'Unable to Predict',1:'Disaster', 2:'Flop',3:'Below Average',4:'Average',5:'Semi Hit',6:'Hit',7:'Super Hit',8:'Blockbuster',9:'All-Time Blockbuster'}
...
def hello_world():
    ...
    if prediction > 0:
           ...
           return jsonify({"Prediction":'Unable to Predict',"Rating":'Not Applicable'})
...
def getTestData(actorsSelected,directorsSelected,movieTestDir):
    movieActorRatingUpdated  = pd.merge(movieTestDir,actorsSelected[actorsSelected.columns],left_on = 'actor',right_on = 'actorName',how = 'left')
    movieDirRatingUpdated  = pd.merge(movieActorRatingUpdated,directorsSelected[directorsSelected.columns],left_on = 'director',right_on = 'directorName',how = 'left')
    movieDirRatingUpdated = movieDirRatingUpdated.fillna(movieDirRatingUpdated.mean())
    movieDirRatingUpdated = movieDirRatingUpdated.fillna(0)
    movieDirRatingUpdated = movieDirRatingUpdated.drop(movieDirRatingUpdated[['actor','actorName','director','directorName']],axis =1)
