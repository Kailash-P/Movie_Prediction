...
function Predict() {
    ...
    var jsonData = { data: [uniqueId, movieTitle, movieReleaseYear, movieReleaseDate, movieGenre, movieWriters, movieActors, movieDirectors, movieSequel] }

    var strJsonData = JSON.stringify(jsonData);

    if (!Validate(uniqueId, movieTitle, movieReleaseYear, movieReleaseDate, movieGenre, movieWriters, movieActors, movieDirectors, movieSequel)) {
        var settings = {
            "async": true,
            "crossDomain": true,
        ...
        $.ajax(settings).done(function (response) {
            if (response) {
                ...
                if (starCount < 5) {
                    var starsToPrint = starCount;
                    for (i = 0; i < (5 - starsToPrint); i++) {
